...
with tab4:
    ...
    with r1c1:
        ...
        try:
            ...
            fcast_merged.columns = [c.strip() for c in fcast_merged.columns]
            fcast_ind_agg = fcast_merged.groupby('INDUSTRY', as_index=False)['DEMAND_FORECAST'].mean()
    ...
    with r1c2:
        ...
        try:
            trend_merged = forecast.merge(skus[['SKU_ID','INDUSTRY']], on='SKU_ID', how='inner')
            trend_merged.columns = [c.strip() for c in trend_merged.columns]
            trend_data = trend_merged.groupby('INDUSTRY', as_index=False)['TREND'].mean()
